...
(function($) {
	var pluginName = 'jqTimeline',
		defaults = {
			startHour : 0 , // Start with one less hour by default
			numHours : 24,
			gap : 15, // gap between lines
			showToolTip : true,
			groupEventWithinPx : 0, // Will show common tooltip for events within this range of px
			events : [],
	...
	function jqTimeLine(element, options) {
		...
		this._current_offset_x = 14; // var used for laying out minutes to the hor line
		this._gap = this.options.gap; 
		this._eDotWidth = 4; // Width of the event dot shown in the ui
	...
	jqTimeLine.prototype._generateMarkup = function() {
		var _this = this;
		var i = 0,j=0;
		var totalWidth = _this.options.numHours * this._gap * 4 + 3;
		...
		for(j=0;j<_this.options.numHours;j++){
			for(i=0;i<4;i++){
				$mainContainer.append(_this._getminuteMarkup(i,_this.options.startHour + j));
			}
		}
		$mainContainer.append(_this._getminuteMarkup(0,_this.options.startHour + _this.options.numHours));
		//Start adding events
		for(var k=0;k<_this.options.events.length;k++){
			var e = _this.options.events[k];
			var d = e.on;
			if(d.getHours() >= _this.options.startHour && d.getHours() < _this.options.startHour + _this.options.numHours){
	...
	jqTimeLine.prototype._getminuteMarkup = function(num,hour){
		var _this = this;
		var retStr = "";
		...
		if(num== 0){
			...
			retStr = '<div class="horizontal-line minute-line odd-minute" style="left:'+_this._current_offset_x+'px"></div>';
		}else{
			retStr = '<div class="horizontal-line minute-line even-minute" style="left:'+_this._current_offset_x+'px"><div class="minute">'+aDelimiters[num]+'</div></div>';
		}
	...
	jqTimeLine.prototype._getEventMarkup = function(e){
		...
		var s = d.getSeconds();
		var h = d.getHours() - _this.options.startHour;
		var m = d.getMinutes();

		var totalHours = (h * 4);
		var leftVal = Math.ceil(_this._offset_x + totalHours * _this.options.gap + (_this.options.gap/15)*m - _this._eDotWidth);
			...
			$retHtml.mouseover( 
				function(e){
					...
					for (var i = 0; i < neighborEvents.length; i++) {
						...
						// var strTime = oData.on.toLocaleTimeString()
						// alert(oData.on)
						strToolTip = strToolTip + '<div class="msg" id="msg_'+oData.id+'">'+oData.on.toLocaleTimeString()+'</div>';
					};
